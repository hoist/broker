  ...
  _createClass(Publisher, [{
    ...
    value: function _savePayloadToS3(event) {
      ...
      return Promise.resolve(_uuid2['default'].v4()).then(function (payloadId) {
        _this2._logger.info({ bucketName: _this2._payloadBucketName }, 'looking up bucket');
        var payload = JSON.stringify(event.payload);
        return _this2._s3Client.headBucketAsync({
          Bucket: _this2._payloadBucketName
        })['catch'](function (err) {
          _this2._logger.error(err);
          _this2._logger.info({ bucketName: _this2._payloadBucketName }, 'creating bucket');
          return _this2._s3Client.createBucketAsync({
            Bucket: _this2._payloadBucketName,
            ACL: 'private'
          });
        }).then(function () {
          _this2._logger.info({ bucketName: _this2._payloadBucketName }, 'uploading payload');
