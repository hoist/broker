...
class Publisher {
  ...
  _openChannel() {
    ...
    if (this.channel) {
      this.logger.debug('reusing existing channel');
      ...
      return Promise.resolve(amqp.connect(config.get('Hoist.rabbit.url'), {
        ...
        .then((connection) => {
          ...
          this.logger.info('got a connection, creating channel');
          ...
          this.logger.debug('channel open');
          this.logger.info('returning channel');
