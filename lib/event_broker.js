...
EventBroker.prototype = {
  send: function (ev, callback) {
    ...
    return this.messageBus.send(ev)
      .then(function () {
        if (ev.constructor.name === 'ApplicationEvent') {
          var log = new EventBroker.ModelResolver.get().ExecutionLogEvent({
            application: ev.applicationId,
            environment: ev.environment,
            type: 'EVT',
            moduleName: ev.eventName,
            message: 'event ' + ev.eventName + ' raised (id: ' + ev.eventId + ')'
          });
          log.saveAsync();
        }
      }).nodeify(callback);
