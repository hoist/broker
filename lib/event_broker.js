'use strict';
var azure = require('azure');
var config = require('config');
var StepLogEvent = require('./event_types/step_log_event');
var ErrorLogEvent = require('./event_types/error_log_event');
var BBPromise = require('bluebird');
var _serviceBus;
var _ = require('lodash');
var defaultTopicOptions = {
  EnableDeadLetteringOnMessageExpiration: true,
  EnableDeadLetteringOnFilterEvaluationExceptions: true
};
var EventBroker = {
  listeners: {

  },
  subscriptions: {

  },
  resetServiceBus: function () {
    _serviceBus = null;
  },
  serviceBus: function () {
    return _serviceBus || (_serviceBus = BBPromise.promisifyAll(azure.createServiceBusService(config.azure.servicebus.main.connectionString)));
  },
  subscribe: function (EventType, ruleDescOrCallback, callback) {
    var rules;
    if ((!callback) && ruleDescOrCallback && _.isFunction(ruleDescOrCallback)) {
      callback = ruleDescOrCallback;
      ruleDescOrCallback = null;
    } else {
      rules = ruleDescOrCallback;
    }
    //already subscribed
    if (EventBroker.subscriptions[EventType.name]) {
      return BBPromise.resolve(null).nodeify(callback);
    }
    return EventBroker.serviceBus().createTopicIfNotExistsAsync(EventType.QueueName, defaultTopicOptions)
      .then(function () {

        var subscriptionName = 'All';
        if (rules && rules.subscriptionName) {
          subscriptionName = rules.subscriptionName;
        }
        return EventBroker.serviceBus().getSubscriptionAsync(EventType.QueueName, subscriptionName)
          .catch(function () {

            return EventBroker.serviceBus().createSubscriptionAsync(EventType.QueueName, subscriptionName, defaultTopicOptions)
              .then(function () {

                if (rules) {


                  //delete default rule
                  return EventBroker.serviceBus().deleteRuleAsync(EventType.QueueName, subscriptionName, azure.Constants.ServiceBusConstants.DEFAULT_RULE_NAME)
                    .then(function () {
                      //create new rules
                      if (rules.rules) {
                        return BBPromise.all(_.map(rules.rules, function (rule) {
                          return EventBroker.serviceBus().createRuleAsync(EventType.QueueName, subscriptionName, rule);
                        }));
                      }
                    });
                }
              });
          });
      }).then(function () {
        EventBroker.subscriptions[EventType.name] = setInterval(function () {
          EventBroker.serviceBus().receiveSubscriptionMessage(EventType.QueueName, 'All', {
            timeoutIntervalInS: Math.max(config.timers.subscriptionTimeout / 1000, 1),
            isPeekLock: true
          }, function (err, brokeredMessage) {
            if (!err) {
              var ev = new EventType(brokeredMessage);
              EventBroker.process(ev);
            } else {
              console.warn(err);
            }
          });
        }, config.timers.subscriptionTimeout);
      }).nodeify(callback);
  },
  unsubscribe: function (EventType) {
    /* istanbul ignore else */
    if (EventBroker.subscriptions[EventType.name]) {
      clearInterval(EventBroker.subscriptions[EventType.name]);
      delete EventBroker.subscriptions[EventType.name];
    }
  },
  send: function (ev, callback) {

    return EventBroker.serviceBus().createQueueIfNotExistsAsync(ev.constructor.QueueName, defaultTopicOptions)
      .then(function () {
        return EventBroker.serviceBus().sendQueueMessageAsync(ev.constructor.QueueName, ev.convertToBrokeredMessage());
      }).nodeify(callback);
  },
  publish: function (ev, callback) {
    return EventBroker.serviceBus().createTopicIfNotExistsAsync(ev.constructor.QueueName, defaultTopicOptions).then(function () {
      return EventBroker.serviceBus().sendTopicMessageAsync(ev.constructor.QueueName, ev.convertToBrokeredMessage());
    }).nodeify(callback);
  },
  listen: function (EventType, callback) {
    if (EventBroker.listeners[EventType.name]) {
      return BBPromise.resolve(null).nodeify(callback);
    }
    return EventBroker.serviceBus().createQueueIfNotExistsAsync(EventType.QueueName, defaultTopicOptions)
      .then(function () {
        EventBroker.listeners[EventType.name] = setInterval(function () {
          EventBroker.serviceBus().receiveQueueMessage(EventType.QueueName, {
            timeoutIntervalInS: Math.max(config.timers.listenTimeout / 1000, 1),
            isPeekLock: true
          }, function (err, brokeredMessage) {
            if (!err) {
              var ev = new EventType(brokeredMessage);
              EventBroker.process(ev);
            } else {
              console.warn(err);
            }
          });
        }, config.timers.listenTimeout);

      }).nodeify(callback);
  },
  unlisten: function (EventType) {
    /* istanbul ignore else */
    if (EventBroker.listeners[EventType.name]) {
      clearInterval(EventBroker.listeners[EventType.name]);
      delete EventBroker.listeners[EventType.name];
    }
  },
  process: function (ev) {
    ev.on('createEvent', function (newEvent) {
      EventBroker.send(newEvent);
    }).on('publishEvent', function (newEvent) {
      EventBroker.publish(newEvent);
    }).on('log.step', function (stepName) {
      var logEvent = {
        applicationId: ev.applicationId,
        environment: ev.environment,
        stepName: stepName,
        correlationId: ev.correlationId,
        baseEvent: ev.toJSON()
      };
      EventBroker.send(new StepLogEvent(logEvent));
    }).on('log.error', function (error) {
      var logEvent = {
        applicationId: ev.applicationId,
        environment: ev.environment,
        correlationId: ev.correlationId,
        baseEvent: ev.toJSON(),
        error: error.message || error.toString()
      };
      EventBroker.send(new ErrorLogEvent(logEvent));
    }).on('done', function () {
      EventBroker.serviceBus().deleteMessage(ev.convertToBrokeredMessage(), function () {

      });
    }).process();
  }
};

module.exports = EventBroker;
