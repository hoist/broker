  ...
  _createClass(Receiver, [{
    ...
    value: function _getPayloadFromId(applicationId, payloadId) {
      ...
      return this._s3Client.getObjectAsync({
        ...
        _this2._logger.info('inside payload response');
      ...
      return this._openChannel().then(function (channel) {
        return Promise.all([channel.assertQueue(eventQueue, {
          ...
          return new Promise(function (resolve) {
            channel.consume(eventQueue, function (msg) {
              _this3._logger.info('events received', msg);
              if (msg.fields.routingKey.split('.')[2] === eventName) {
                var message = JSON.parse(msg.content.toString('utf8'));
                _this3._logger.info('matched split');
                _this3.restore(message).then(function (msg) {
                  resolve(msg);
                });
              }
            });
          ...
          _this3._logger.info('closing channel');
        ...
        _this3._logger.info('sending log event');
        return result;
      });
    }
  }]);
