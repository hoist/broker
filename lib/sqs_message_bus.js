...
SQSMessageBus.prototype.getApplicationQueueUrl = function (type, applicationId) {
  var getQueueUrl;
  if (this.applicationQueueUrls[type.name]) {
    getQueueUrl = BBPromise.resolve(this.applicationQueueUrls[type.name])
      .bind(this);
  } else {
    getQueueUrl = this.sqs.createQueueAsync({
      Attributes: this.defaultQueueAttributes,
      ...
      this.applicationQueueUrls[type.name] = data.QueueUrl;
      return data.QueueUrl;
    });
  }
  return getQueueUrl;
